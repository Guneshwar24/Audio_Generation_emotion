...
from keras.layers import (Layer, Input, Flatten, Dropout, BatchNormalization, Reshape,
                          ...
                          Conv2DTranspose, Conv1D, Dense, LeakyReLU, ReLU, SpectralNormalization, Activation,
...
def generator(NoiseDim, OutputShape):
    ...
    # model.add(UpSampling1D(size=2))  # Upsampling
    model.add(ReLU())
    # model.add(BatchNormalization(momentum=0.9))
...
def discriminator(InputShape):
    ...
    # model.add(SpectralNormalization(Conv1D(32, 100, strides=7, padding='valid')))
    model.add(LeakyReLU())
